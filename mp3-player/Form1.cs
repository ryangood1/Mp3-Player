    ...
    public partial class Form1 : Form
    {
        ...
        {      
            if (saveFileDialog1.ShowDialog() == DialogResult.OK) 
            {
                ...
                saveFileDialog1.FileName = playlist.ToString();
                foreach (Song song in playlist.SongList) //Goes through each song in the current playlist
                {
                    writer.WriteLine(song.Songname.PadRight(15) + song.ToString());
                }
                writer.Close();
            }
        }

        private void newPlaylistByRatingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Playlist temp = new Playlist();
            int desiredRating = 0;
            desiredRating = int.Parse(textBoxPlaylistByRating.Text);
            foreach (Song song in playlist.SongList)
            {
                Console.WriteLine(song.rating);
                if (song.rating == desiredRating)
                {
                    temp.AddSong(song);
                }
            }
